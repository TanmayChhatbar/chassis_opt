function [F_edges, F_reaction, F_edges_cartesian] = truss_sim(frame)
%% sanity checks
...
% moment sub-matrix (about constrained vertex 1)
...
% A*x = b
% x = forces along each edge
% b = external + reaction forces
...
for idx_vert = 1:height(frame.vertices)
    ...
    for idx_edge = idx_edges
        ...
        % check which end of the edge this vertex is
        if frame.edges(idx_edge, 1) == idx_vert
...
b = b + b2;
if ~all(sum(b) < 1e-10)
    error("forces not balanced")
end

moment = zeros(1, width(frame.vertices));
for i = 1:height(frame.vertices)
    moment_this = cross(frame.vertices(i, :), b(i, :));
    moment = moment + moment_this;
end
if ~all(moment < 1e-10)
    error("moment not balanced")
end

A = cat(1, A(:,:,1),A(:,:,2),A(:,:,3));
b = reshape(b, [], 1);

%% invert matrix to solve for F_edges
F_edges = A \ b;
if any(A * F_edges - b > 1e-3)
    error("force calculation failed. recheck triangulation")
end

%% calculate force in direction of the edge
F_edges_cartesian = nan(height(frame.edges), 3);
for ei = 1:height(frame.edges)
    edge = diff(frame.vertices(frame.edges(ei, :), :));
    F_edges_cartesian(ei, :) = F_edges(ei) * edge / norm(edge);
end
